 ...
 */
static void fill_session_packet_session_id(struct OptHeader *opt_header, struct SessionID session_id) {
 ...
 */
static void fill_session_packet_path_length(struct OptHeader *opt_header, int path_length) {
    // 获取路径长度起始 pointer
 ...
 * @param rte 路由表项
 */
static void fill_session_packet_path(struct OptHeader *opt_header, struct RoutingTableEntry *rte) {
    // 索引
    int index;
    // 路径起始字段
...
static void fill_session_packet_fields(struct OptHeader *opt_header,
                                       struct RoutingTableEntry *rte,
                                       struct SessionID session_id){
    // 1. 进行会话包的 session_id 填充
    fill_session_packet_session_id(opt_header, session_id);
    // 2. 进行会话包的 path_length 填充
    fill_session_packet_path_length(opt_header, rte->path_length);
    // 3. 进行会话包的 opt_path 填充
    fill_session_packet_path(opt_header, rte);
}
...
struct sk_buff *self_defined__session_make_skb(struct sock *sk,
    ...
    opt_header->version = OPT_ESTABLISH_VERSION_NUMBER; // 版本 (字段1)
    ...
    opt_header->hdr_len = get_session_header_size(rcr); // 设置数据包总长度 (字段10)
    ...
    // 头部后续部分初始化
    // ---------------------------------------------------------------------------------------
    fill_session_packet_fields(opt_header, rcr->rtes[0], session_id);
    // ---------------------------------------------------------------------------------------
