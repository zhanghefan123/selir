 ...
 */
void free_path_validation_structure(struct PathValidationStructure *pvs) {
    if (pvs->routing_table_type == ARRAY_BASED_ROUTING_TABLE_TYPE) {
        // 进行基于数组路由表的释放
        free_array_based_routing_table(pvs->abrt);
    } else if (pvs->routing_table_type == HASH_BASED_ROUTING_TABLE_TYPE) {
        // 进行基于哈希的路由表的释放
        free_hash_based_routing_table(pvs->hbrt);
    } else {
...
void initialize_routing_and_forwarding_table(struct PathValidationStructure *pvs,
    ...
    if (ARRAY_BASED_ROUTING_TABLE_TYPE == routing_table_type) {
        pvs->abrt = initialize_array_based_routing_table(number_of_routes_or_buckets);
