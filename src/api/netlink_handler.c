...
int netlink_init_routing_and_forwarding_table_handler(struct sk_buff *request, struct genl_info *info) {
    ...
    // 3.3 判断 pvs 是否为 NULL
    if (NULL == pvs) {
        LOG_WITH_PREFIX("pvs == NULL");
    }
    // 3.3 创建 path validation structure
    initialize_routing_and_forwarding_table(pvs, number_of_routes, number_of_interfaces);
    // -----------------------------------------------------------------
...
int netlink_insert_routing_table_entry_handler(struct sk_buff* request, struct genl_info* info){
    ...
    // 3.2 获取 pvs
    struct PathValidationStructure* pvs = get_pvs_from_ns(current_ns);
    pvs->interface_table[index].link_identifier = link_identifier;
    pvs->interface_table[index].interface = dev_get_by_index(current_ns, ifindex);
    dev_put(pvs->interface_table[index].interface);
    // -----------------------------------------------------------------
    ...
    snprintf(response_buffer, sizeof(response_buffer),
             ...
             index, link_identifier, ifindex, pvs->interface_table[index].interface->name);
    return send_reply(response_buffer, info);
    // -----------------------------------------------------------------
}
...
int netlink_insert_interface_table_entry_handler(struct sk_buff* request, struct genl_info* info){
    ...
    // 链路标识: [1,2]
    // 节点标识: [b,c]
    // 消息格式: index, source_id, destination_id, length_of_path
    // 3.1 读取参数
    receive_buffer = recv_message(info);
    while (true) {
        // 分割出来的字符串
    ...
    // -----------------------------------------------------------------
    snprintf(response_buffer, sizeof(response_buffer),
             "index: %d, source_id: %d, destination_id: %d, path_length: %d, link_identifier_index: %d, node_index: %d",
             index, source_id, destination_id, path_length, link_identifier_index, node_index);
    return send_reply(response_buffer, info);
    // -----------------------------------------------------------------
}