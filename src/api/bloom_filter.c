...
void test_bloom_filter(void) {
    ...
    bloom_filter_tmp->bitset_mask = 0x3F; // 3 * 16 + 15 = 63
    bloom_filter_tmp->hash_seed = 0x12; // 哈希种子
    bloom_filter_tmp->aligned_u32_count = 0x0; // 对齐
    bloom_filter_tmp->nr_hash_funcs = 0x05; // 哈希函数个数
    // 3. 开始进行测试
    // --------------------------------------------------------------------
    ...
    struct bloom_filter* bloom_filter_copy = copy_bloom_filter(bloom_filter_tmp);
    // 3.9 再次进行 check
    for(index = 0; index < 3; index++){
        int checkElement = allElements[index];
